...
def query_3():
    ...
    has_class_a_car=has_class_a_car
    ).filter(has_class_a_car=True).count()
    return q
...
def query_5(t):
    q = Rider.objects.annotate(total_payment = Sum('riderequest__ride__payment__amount')).filter(total_payment__gte = t)
    return q


def query_6():
    q = Driver.objects.values('account__last_name').annotate(car_amount = Count('car')).order_by('-car_amount','account__last_name')
    return q
...
def query_10():
    q = Driver.objects.values('account__first_name').annotate(number_of_trips = Count('car__ride')).order_by('account__first_name').distinct()
    return q
...
def query_13(n, m):
    q = Ride.objects.prefetch_related('car').filter(car__owner__account__first_name = n , request__rider__account__first_name = m).annotate(sum_duration = Sum(F('dropoff_time') - F('pickup_time')))
